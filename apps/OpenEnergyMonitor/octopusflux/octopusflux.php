...
<div id="app-block" style="display:none">
  ...
  <div id="octopus-realtime" class="col1"><div class="col1-inner">
    ...
    <div class="block-bound">
      ...
      <div id="app-title" class="block-title">OCTOPUS FLUX</div>
    </div>
        ...
        <div class="row-fluid">
            <div class="span7 appconfig-description">
                <div class="appconfig-description-inner text-light">
                    <h2 class="appconfig-title text-primary"><?php echo _('Octopus Flux'); ?></h2>
                    <p class="lead">Explore Octopus Flux tariff costs over time.</p>
                    ...
                    <img src="../Modules/app/images/flux_app.png" class="d-none d-sm-inline-block">
                </div>
            </div>
...
// ----------------------------------------------------------------------
config.app = {
    "title":{"type":"value", "default":"OCTOPUS FLUX", "name": "Title", "description":"Optional title for app"},
    ...
    "tariff":{"type":"select", "name":"Select tariff:", "default":"FLUX", "options":["FLUX"]},
 
    "region":{"type":"select", "name":"Select region:", "default":"Z_Spoofed", "options":["Z_Spoofed"]},
...
octopus_feed_list["FLUX"] = {};
octopus_feed_list["FLUX"]["Z_Spoofed"] = 10001;
...
$('#placeholder').bind("plothover", function (event, pos, item) {
    if (item) {
        ...
        if (previousPoint != item.datapoint) {
            ...
            var text = "";
            if (profile_mode) {
                if (item.series.label=='Flux' || item.series.label=='Outgoing') {
                    text += "Average ";
                } else {
                    text += "Cumulative ";
                }
...
$("#download-csv").click(function(){
    ...
    download_data("flux-data.csv",csv.join("\n"));    
});
...
function graph_load() 
{
    ...
    if (config.app.region!=undefined && octopus_feed_list[config.app.tariff.value][config.app.region.value]!=undefined) {
        ...
        flux = getdataremote(octopus_feed_list[config.app.tariff.value][config.app.region.value],view.start,view.end,interval);
        for (var z in flux) {
            data["flux"].push(flux[z]);
            data["flux"].push([flux[z][0]+(intervalms-1), flux[z][1]]);
        }
...
function graph_draw() 
{
    ...
    if (this_halfhour_index!=-1) {
        ...
        let unit_price = data["flux"][2*this_halfhour_index][1]*1.05;
    ...
    // price signals
    graph_series.push({label: "Flux", data:data["flux"], yaxis:2, color:"#fb1a80", lines: { show: true, align: "left", lineWidth:1}});
...
function profile_draw() 
{
    ...
    graph_series.push({label: "Flux", data:profile_unitprice, yaxis:2, color:"#fb1a80", lines: { show: true, align: "left", lineWidth:1}});
...
// Remote data request
// ----------------------------------------------------------------------
function getdataremote_feed(id,start,end,interval)
{   
    var data = [];
    $.ajax({                                      
...
function getdataremote(id,start,end,interval)
{
    ...
    const li = 18.73333333333333;
    const di = 31.22857142857143;
    const hi = 43.71428571428571;
    const ol = 8.67;
    const od = 21.79;
    const oh = 34.9;
    ...
    var imp = [di,di,li,li,li,di,di,di,di,di,di,di,di,di,di,di,hi,hi,hi,di,di,di,di,di];
    var exp = [od,od,ol,ol,ol,od,od,od,od,od,od,od,od,od,od,od,oh,oh,oh,od,od,od,od,od];
    ...
    do {
        ...
        if (id == 10001)
            data.push([start, imp[hours]]);
        else
            data.push([start, exp[hours]]);
